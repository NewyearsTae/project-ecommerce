...
if (isset($_POST["submit"])){
    ...
    $errors =  array();
    if (empty($u_fullname) OR empty($u_username) OR empty($u_password) OR empty($u_password2)){
    ...
    if(count($errors)>0){
        ...
        $sql = "INSERT INTO user (u_fullname, u_username, u_password, u_level) VALUE ( ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($objCon);
        $prepareStmt = mysqli_stmt_prepare($stmt,$sql);
    if ($prepareStmt) {
        mysqli_stmt_bind_param($stmt,"ssss",$u_fullname, $u_username, $passwordHash ,$u_level);
