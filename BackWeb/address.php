...
if (isset($_POST["submit"])) {
    $u_address = $_POST["u_address"]; // เพิ่มการรับค่าที่อยู่จากฟอร์ม
    $Phonenumber =$_POST["Phonenumber"];

    $sql = "UPDATE user SET u_address = ?, Phonenumber = ? WHERE u_id = ?";
    $stmt = mysqli_stmt_init($objCon);
    $prepareStmt = mysqli_stmt_prepare($stmt,$sql);

    if ($prepareStmt) {
        mysqli_stmt_bind_param($stmt, "ssi", $u_address,$Phonenumber,$id);
    ...
    // ส่งกลับไปยังหน้าที่คุณต้องการหลังจากการอัปเดตที่อยู่ของผู้ใช้
    header("location: Admin.php");
}
...
<body>
    <div class="">
        ...
        <div class="content">
            ...
            <!-- Navbar End -->
        <form method="post" action="address.php?user_id=<?php echo $row["u_id"]; ?>">
            <div class="bgeditmain">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="u_address" name="u_address" placeholder="Address" required ><?php echo $row["u_address"]; ?></textarea>           
                </div>
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="Phonenumber" name="Phonenumber" placeholder="Phonenumber" value="<?php echo $row["Phonenumber"]; ?>">
