        ...
        <div class="content">
            ...
            <div class="bgbgbg">
                ...
                <div class="button-container">
                    ...
                    <button id="button3" class="button active">Month</button>
                    ...
                    <div class="table-responsive">
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                // หาวันที่สุดท้ายของเดือนปัจจุบัน
                                $lastDayOfMonth = date('Y-m-t');

                                // ค้นหาข้อมูลในเดือนปัจจุบัน
                                $sql_order = "SELECT DAY(created_at) AS day, COUNT(order_id) AS quantity, SUM(price) AS total_price, SUM(sc) AS total_shipping_cost FROM orderhistory WHERE created_at >= '$firstDayOfMonth' AND created_at <= '$lastDayOfMonth' GROUP BY DAY(created_at)";
                                ...
                                if ($result_order->num_rows > 0) {
                                    while ($row = $result_order->fetch_assoc()) {
                                        echo '<tr>';
                                        echo '<td>' . $row["day"] . '</td>'; // แสดงวันที่
                                        echo '<td>' . $row["quantity"] . '</td>';
                                        echo '<td>' . number_format($row["total_price"], 2) . '</td>';
                                        echo '<td>' . number_format($row["total_shipping_cost"], 2) . '</td>';
                                        echo '</tr>';
                                    }
                                } else {
                                    // ถ้าไม่มีข้อมูลให้แสดงข้อความว่า "No data available"
