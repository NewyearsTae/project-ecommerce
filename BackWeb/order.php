...
$sql = "SELECT * FROM orders  WHERE orders.status = '1' ";
$result = mysqli_query($objCon, $sql);
$total_order = mysqli_num_rows($result);

$sql = "SELECT orders.*, user.u_fullname 
        FROM orders
        INNER JOIN user ON orders.user_id = user.u_id
        WHERE orders.status = '1'
        ORDER BY orders.created_at DESC"; // เพิ่ม ORDER BY เพื่อเรียงลำดับข้อมูลจากใหม่สุดก่อน
$result = mysqli_query($objCon, $sql);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="image-container">
                                                <i class="fa fa-file-alt fa-3x text-primary" alt="Icon Image" class="icon-image" onclick="showImage('<?php echo '/Webpage/receipt/' . $row['receipt']; ?>')"></i>
                                                <div id="overlay" class="overlay" onclick="hideImage()">
                                                    <img id="expanded-image" src="" alt="Receipt Image" class="expanded-image">
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="orderno.php?order_id=<?php echo $row["order_id"]; ?>" class="btn btn-info">Details</a>
                                        </td>
                                    </tr>
