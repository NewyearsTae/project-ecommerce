...
<body>
    <div class="">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="product.php" class="nav-item nav-link active"><i
                            class="fa fa-box-open me-2"></i>Product</a>
        ...
        <div class="content">
            ...
            <?php } ?>
                ...
                <select class="form-control" id="type" name="type" required onchange="toggleCustomInput()">
                    <option value="">Status</option>
                    <option value="Complete">Sold Out</option>
            ...
            $result_total_product = mysqli_query($objCon, $sql_total_product);
            $total_product = mysqli_num_rows($result_total_product);

            // ดึงข้อมูลตามหน้าที่เลือก
            $sql = "SELECT * FROM product ORDER BY id ASC LIMIT $offset, $items_per_page";
            $result = mysqli_query($objCon, $sql);
            ...
            while ($row2 = mysqli_fetch_assoc($result)) {
                ...
                echo '<td>' . $row2['lot'] . '</td>';
                if ($row2['num'] == 1) {
                    echo '<td><center><div class="textstatuscomplete"><center><p>Sold Out<p><center></div><center></td>';
                } elseif ($row2['num'] == 3) {
                    echo '<td><center><div class="textstatusdamage"><center><p>Damage<p><center></div><center></td>';
                } else {
                    echo '<td><center><div class="textstatusempty"><center><p>Selling<p><center></div><center></td>';
                }
                echo '</td>';
                echo '</tr>';
                $start_number++;
    ...
    <?php
    // ตรวจสอบจำนวนข้อมูลทั้งหมดในตาราง product
    $total_records = mysqli_num_rows(mysqli_query($objCon, "SELECT * FROM product"));
    $items_per_page = 30;
    ...
    // แสดงปุ่ม Previous ถ้าไม่ใช่หน้าแรก
    if ($page > 1) {
        echo '<a class="btnpage" href="?page=' . ($page - 1) . '">Previous</a>';
    }

    // แสดงเลขหน้า ถ้าข้อมูลมากกว่า 30 รายการ
    if ($total_pages > 1) {
    ...
    // แสดงปุ่ม Next ถ้าไม่ใช่หน้าสุดท้าย
    if ($page < $total_pages) {
