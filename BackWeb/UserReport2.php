...
$sql = "SELECT complain.*, 
               ...
               complaindetails.description AS description2 
        FROM complain 
        INNER JOIN user ON complain.user_id = user.u_id
        INNER JOIN orderhistory ON complain.order_id = orderhistory.order_id
        INNER JOIN complaindetails ON complain.complain_id = complaindetails.complain_id
        WHERE complain.complain_id = ?
        GROUP BY complain.complain_id, user.u_fullname, user.email, user.Phonenumber, user.u_address, complaindetails.description";
        ...
        <!-- Spinner End -->
         ...
         <div class="reportbg">
            <div class="userdetail">
                ...
                <p style="margin-bottom: 20px;"><?php echo $row['u_fullname']; ?><p>
                </div>
                <div class="bgbox">
            ...
            <div class="reportdetail">
                ...
                <label for="complainDetailTextarea" class="form-label"></label>
                <textarea class="form-control" id="complainDetailTextarea" name="complainDetailTextarea" rows="10" style="width: 90%; height:15vw; padding: 10px; border-radius: 20px; margin-left: 4%;" readonly>
                    <?php echo htmlspecialchars($row['description2']); ?>
                </textarea>
                <br>
                <a href="complaindetails.php" class="btn btn-primary" style="margin-top: 10px; margin-right: 10px; float: right;">กลับ</a>
            </div>
