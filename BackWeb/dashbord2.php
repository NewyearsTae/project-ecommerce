...
$sql_price = "SELECT SUM(price) AS total_price_after_damage 
              FROM orderhistory 
              WHERE created_at >= DATE_SUB(CURDATE(), INTERVAL DAYOFWEEK(CURDATE()) - 2 DAY) 
              AND created_at < DATE_ADD(CURDATE(), INTERVAL 7 - DAYOFWEEK(CURDATE()) DAY)";
        ...
        <div class="content">
            ...
            <div class="bgbgbg">
                ...
                <div class="button-container">
                    ...
                    <button id="button1" class="button active">Week</button>
                    ...
                    <div class="table-responsive">
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                // วนลูปแสดงผลตาราง 7 วัน
                                for ($i = 0; $i < 7; $i++) {
                                    // กำหนดวันที่เริ่มต้นและสิ้นสุดของแต่ละวัน
                                    $startDate = date('Y-m-d', strtotime('last Monday +' . $i . ' day'));
                                    $endDate = date('Y-m-d', strtotime('last Monday +' . ($i + 1) . ' day'));

                                    // ค้นหาข้อมูลในช่วงเวลานี้
                                    $sql_order = "SELECT DATE(created_at) AS order_date, COUNT(order_id) AS quantity, SUM(price) AS total_price, SUM(sc) AS total_shipping_cost FROM orderhistory WHERE created_at >= '$startDate' AND created_at < '$endDate' GROUP BY DATE(created_at)";
                                    $result_order = mysqli_query($objCon, $sql_order);

                                    // แสดงผลลัพธ์ของแต่ละวัน
                                    echo '<tr>';
                                    echo '<td>' . date('l', strtotime($startDate)) . '</td>'; // แสดงวันที่ในรูปแบบของวันในสัปดาห์
