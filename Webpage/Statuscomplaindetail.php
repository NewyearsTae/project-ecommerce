...
$sql = "SELECT cd.order_id, oh.company, oh.tracking, oh.price, oh.created_at, c.image, c.description AS description1, cd.description AS description2, cd.status, cd.accept
        FROM complaindetails cd
        INNER JOIN complain c ON cd.complain_id = c.complain_id
        INNER JOIN orderhistory oh ON cd.order_id = oh.order_id
        WHERE cd.complain_id = '$complain_id'";

// สั่งให้ฐานข้อมูลทำงานกับคำสั่ง SQL
...
<div class="historybg">
    ...
    <div class="toppichis" ><h1>Status Complain</h1></div>
    <div class="historyitem2">

    <!-- item1 -->
    <div class="bgcomplainstatus">
          <div class="complainstatus">
            <!-- orderid -->
            <div class="text">
            <h4 style="margin-right: 20px;">Order ID :</h4> 
            <h4 ><?php echo $row['order_id']; ?></h4>
            </div>
            <!-- orderid -->
            <!-- Company Shipping -->
            <div class="text">
            <h4 style="margin-right: 20px;">Company Shipping :</h4>
            <h4 ><?php echo $row['company']; ?></h4>
            </div>
            <!-- Company Shipping -->
            <!-- Tracking Number -->
            <div class="text">
            <h4 style="margin-right: 20px;">Tracking Number :</h4>
            <h4 ><?php echo $row['tracking']; ?></h4>
            </div>
            <!-- Tracking Number -->
            <!-- Price -->
            <div class="text">
            <h4 style="margin-right: 20px;">Price :</h4>
            <h4 style="margin-right: 5px;"><?php echo $row['price']; ?></h4>
            <h4 >THB</h4>
            </div>
            <!-- Price -->
            <!-- Price -->
            <div class="text">
            <h4 style="margin-right: 20px;">DATE :</h4>
            <h4 style="margin-right: 5px;"><?php echo $row['created_at']; ?></h4>
            </div>
            <!-- Price -->
          </div>
          ...
          <div class="complainstatus">
            <!-- img -->
            <div class="text">
            <h4 style="margin-right: 20px;">IMG :</h4>

            <div class="imgloop">
            <img class="imgsize" src="/Webpage/complain/<?php echo $row['image']; ?>" >
            </div>
            </div>
            <!-- img --> 
            <!-- Descriptions -->
            <div class="text" style="margin-top: 10px;">
            <h4 style="margin-right: 20px;">Descriptions:</h4>
            <h4><?php echo $row['description1']; ?></h4>
            </div>
            <!-- Descriptions -->  
          </div>       
        </div>
        ...
        <!-- ssssssssssssssssssss -->
          <div class="complainstatus">
            <div class="text">
            <h4 style="margin-right: 20px;">Status : <?php echo $row['status']; ?></h4>
            </div>
            <!-- Descriptions -->
            <div class="text" style="margin-top: 10px;">
            <h4 style="margin-right: 20px;">Descriptions:</h4>
        ...
        <div class="btnbgbg">
            <?php
            if ($row['accept'] == 0) {
                echo '<a href="comfirmcomplain.php?complain_id=' . $complain_id . '&user_id=' . $user_id . '&isLoggedIn=true" class="btn btn-primary" style="margin-top: 10px; float: right;">Accept</a>';
            } elseif ($row['accept']  == 1) {
                echo '<a href="Statuscomplain.php?isLoggedIn=true&user_id=' . $user_id . '" class="btn btn-primary" style="margin-top: 10%; margin-right: 10%; float: right;">Back</a>';
            }
            ?>
        </div>   
        </div> 
    ...
    <div class="offcanvas-body">
        ...
        while($row=mysqli_fetch_assoc($result_cart)){
          ?>          
                  ...
                  <div><button type="button" class="btn btn-outline-danger btn-sm" onclick="deleteCartItem(<?php echo $row['id']; ?>); reloadPage();">X</button>
                  </div>
                  </div>
         <?php } ?>
        ...
        <div class="btncartbuy" style="font-family: fantasy; color: white; font-size: 1.1vw;">
          <a href="allcart.php?isLoggedIn=true&user_id=<?php echo $user['id']; ?>" style="color: white; text-decoration: none;">Buy</a>
        </div>
    </div>
</div>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="cartSidebarII" aria-labelledby="cartSidebarLabel">
    ...
    <div class="offcanvas-body">
        ...
        while($row=mysqli_fetch_assoc($result_orders)){
          ?>      
                  <?php echo '<a href="ConfirmOrders.php?isLoggedIn=true&user_id=' . $user_id . '&order_id=' . $row['order_id'] . '">' ?>
                  <?php echo '<div class="ordershow">'?>
