...
<section id="allcart-page">
    <div class="container">
      <div class="row">
      <div class="col-sm-12">
  <br>
  <div class="alert alert-secondary" role="alert" style="display:flex; justify-content: space-between;">
      <!-- สถานะการสั่งซื้อสินค้า -->
      <?php
        // Query to get user details
        $query = "SELECT u_address, Phonenumber, email FROM user WHERE u_id = ?";
        $stmt = $objCon->prepare($query);

        if ($stmt) {
          $stmt->bind_param("i", $user_id); // The number of variables must match the number of parameters in the prepared statement
          $stmt->execute();
          ...
          if ($result->num_rows > 0) {
              ...
              if (empty($u_address) || empty($Phonenumber) || empty($email)) {
                  echo "Some information is missing. Please update your profile.<br>";
                  echo '<a href="profile.php?isLoggedIn=true&user_id=' . $user['id'] . '" class="btn btn-primary">Update Profile</a>';
              } else {
                  // Display user details
                  echo "Address: " . htmlspecialchars($u_address) . "<br>";
      ...
      ?>
      <div class="payment">
        <h5>SCB : 4380300904</h5>
        <img class="imgpayment" src="images/qrcode.jpg">
      </div>
  </div>
  </div>
        ...
        <div class="col-2 col-sm-2">
          <button class="btn btn-secondary" style="width: 100%;" > ชำระ <br> เงินแล้ว </button>
        </a>
        </div>
        <div class="col-1 col-sm-1" style="margin-top: 15px;">
          <ion-icon name="arrow-forward-outline" size="large"></ion-icon>
        ...
        <div class="col-2 col-sm-2">
          <button class="btn btn-secondary" style="width: 100%;"> ตรวจสอบเลข <br> Tracking </button>
        </div>
        <div class="col-1 col-sm-1" style="margin-top: 15px;">
          <ion-icon name="arrow-forward-outline" size="large"></ion-icon>
        </div>
        <div class="col-2 col-sm-2">
          <button class="btn btn-secondary" style="width: 100%;"> จัดส่ง <br> สำเร็จ </button>
        </div>
      </div>
    </div>
    <div class="container">
...
<div class="mb-3">
    <form action="receipt.php?user_id=<?php echo $user_id; ?>&isLoggedIn=<?php echo $isLoggedIn; ?>" method="post" enctype="multipart/form-data">
        <input type="hidden" name="totalPrice" value="<?php echo $totalPrice; ?>">
        <div class="imgctrl">
        <div class="mb-3">
            <input type="file" class="form-control" id="receipt" name="receipt" style="display: none" accept="image/*" onchange="previewReceipt('receipt', 'imagePreviewContainer_main', 'selectedFileNames_main')">
            <label for="receipt" class="btn btn-primary">Upload receipt</label>
            
            <div id="selectedFileNames_main"></div>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="cartSidebar" aria-labelledby="cartSidebarLabel">
    ...
    <div class="offcanvas-body">
        ...
        <div class="btncartbuy" style="font-family: fantasy; color: white; font-size: 1.1vw;">
          <a href="allcart.php?isLoggedIn=true&user_id=<?php echo $user['id']; ?>#allcart-page" style="color: white; text-decoration: none;">Buy</a>
        </div>
    </div>
</div>
