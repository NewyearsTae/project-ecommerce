...
if (isset($user_id) && !empty($user_id)) {
  ...
  $sql_order_line = "SELECT order_line.*, product.name, product.size, product.price, product.img
                     FROM order_line 
                     LEFT JOIN product ON order_line.product_id = product.id 
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col-2 col-sm-2">
          <button class="btn btn-success" style="width: 100%;"> รอชำระ <br> เงิน </button>
          </a>
        </div>
        ...
        <div class="col-2 col-sm-2">
          <a
            href="Trackingpage.php?isLoggedIn=true&order_id=<?php echo $order_id; ?>&user_id=<?php echo $user['id']; ?>#tracking-page">
        ...
        <div class="col-sm-12">
          ...
          <div class="table-responsive">
            ...
            <div class="col-sm-12">
              <br>
              <?php
              // ตรวจสอบค่า Status จากตาราง orders
              $sql_status = "SELECT Status FROM orders WHERE order_id = ?";
              $stmt_status = mysqli_prepare($objCon, $sql_status);
              mysqli_stmt_bind_param($stmt_status, "i", $order_id);
              mysqli_stmt_execute($stmt_status);
              mysqli_stmt_store_result($stmt_status);

              if (mysqli_stmt_num_rows($stmt_status) > 0) {
                mysqli_stmt_bind_result($stmt_status, $status);
                mysqli_stmt_fetch($stmt_status);

                // ตรวจสอบค่า Status
                if ($status === "3") {
                  // แสดงข้อความเมื่อยืนยันรายการสินค้าแล้ว
                  echo '<div class="alert alert-success d-flex justify-content-between" role="alert">';
                  echo 'ยืนยันรายการสินค้าแล้ว';
                  echo '<a href="Trackingpage.php?order_id=' . $order_id . '&user_id=' . $user_id . '&isLoggedIn=true#tracking-page" class="btn btn-primary">ไปยังหน้า Tracking</a>';
                  echo '</div>';
                } else {
                  // แสดงข้อความเมื่อรอทำรายการ
